...
void montarListaEncadeadaComCabecote(Lista *lista){
    ...
    for(int i=0;i<10;i++)
    {
        for(int j=0;j<tamanhoEstrutura[i];j++)
        {
            No *novo = malloc(sizeof(No));
            novo->valor = vetorPrincipal[i][j];
            novo->proximo = NULL;
...
Retorno void
*/
void getDadosListaEncadeadaComCabecote(Lista *lista, int vetorAux[]){
